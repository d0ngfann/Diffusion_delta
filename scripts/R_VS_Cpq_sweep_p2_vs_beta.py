# python3 R_VS_Cpq_sweep_p2_vs_beta.py --p 2 --q 4 --thrshld 2.0 --n-pairs 100
import os
import sys
import time
import math
...
def generate_parameter_combinations(p, q, thrshld):
    """
    Generate all valid parameter combinations for the p2 vs beta sweep.

    Parameters:
        p: int - Cycle-power parameter
        q: float - Random component parameter
        thrshld: float - Social reinforcement threshold (fixed for this sweep)
    """
    n_values = [2000]
    k_values = [8]
    p1_values = [0.01]
    thrshld_values = [thrshld]
...
def save_results(summary_df, full_df, timestamp, p, q, thrshld, output_dir='../output_data'):
    os.makedirs(output_dir, exist_ok=True)
    suffix = f"p{p}_q{q}_p2_vs_beta_thrshld{thrshld}"
...
def main():
    parser = argparse.ArgumentParser(description='Sweep p2 vs beta for R vs C_p,q/n analysis')
    ...
    parser.add_argument('--thrshld', type=float, required=True, help='Social reinforcement threshold (fixed for this sweep)')
    ...
    print("="*70)
    print("DELTA-STAR SWEEP: p2 vs beta")
    print(f"Timestamp: {timestamp}")
    print(f"Fixed thrshld: {args.thrshld}")

    combinations = generate_parameter_combinations(args.p, args.q, args.thrshld)
    summary_df, full_df, _ = run_parameter_sweep(combinations, args.n_pairs, n_workers)

    if not summary_df.empty:
        save_results(summary_df, full_df, timestamp, args.p, args.q, args.thrshld)
    print("\nSWEEP COMPLETE!")

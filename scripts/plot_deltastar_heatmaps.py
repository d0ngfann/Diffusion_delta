"""
Generate discrete delta-star (δ*) heatmap plots for parameter sweep results

This script processes summary CSV files from output_data/20260101/ and generates
...
- Outputs to: output_plots/20260101/

Usage:
    cd scripts
    python plot_deltastar_heatmaps.py
"""
...
# Configuration
DATA_DIR = os.path.join(PROJECT_ROOT, 'output_data', '20260101')
OUTPUT_DIR = os.path.join(PROJECT_ROOT, 'output_plots', '20260101')
...
def parse_filename(filename):
    ...
    Parameters:
        filename: CSV filename (e.g., '20251231_161635_sweep_summary_p2_q4.0_p2_vs_thrshld_beta1.0.csv')
...
def plot_case_distribution_single_file(csv_path, file_info, output_path):
    ...
    title = f'Case Distribution: {x_param} vs {y_param} ({fixed_str}) | C_{{{p},{q:.0f}/n}}'
...
def plot_contour_single_file(csv_path, file_info, output_path):
    ...
    title = f'δ* Heatmap: {x_param} vs {y_param} ({fixed_str}) | C_{{{p},{q:.0f}/n}}'
    ax.set_title(title, fontsize=14, fontweight='bold')
    ...
    # Add annotation
    textstr = f'Discrete Heatmap: {len(x_unique)}×{len(y_unique)} parameter combinations\nDashed lines on colorbar: δ* = 0.3, 0.4, 0.5, 0.6, 0.7'
    props = dict(boxstyle='round', facecolor='white', alpha=0.85, edgecolor='gray')
    ax.text(0.02, 0.98, textstr, transform=ax.transAxes, fontsize=9,
            verticalalignment='top', bbox=props)
...
def main():
    ...
    """
    print("="*70)
    print("DELTA-STAR HEATMAP GENERATOR")
    print("Settings:")
    ...
    # Print summary
    print("\n" + "="*70)
    print("DELTA-STAR HEATMAP GENERATION COMPLETE!")
    print("="*70)
    print(f"\nGenerated plots:")
